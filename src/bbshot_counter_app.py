...
def main(path, disp=False, save_vid=False,debug = False):
...
if __name__ == "__main__":
    ...
    if args.vid_path or (len(sys.argv) > 1 and sys.argv[1] != '--debug'):
        ...
        main(vid_path, args.disp, save_vid=args.save,debug = args.debug)
    else:
        if args.debug:
        ...
        if any(file.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.flv')) for file in os.listdir(current_directory)):
            main(current_directory, disp=args.disp,save_vid=True)
        else:
            ...
            main(default_path_, True, save_vid=False,debug = args.debug)